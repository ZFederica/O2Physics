...
struct HfTaskMcEfficiencyToXiPi {
  ...
  enum HFStep { kHFStepMC = 0,
                kHFStepMcInRapidity,        // MC mother in rapidity |y| < rapidityCharmBaryonMax=0.5 (loop over gen)
                kHFStepAcceptance,          // MC mother where all final state candidates pass eta and pt selection (loop over gen)
                kHFStepTrackable,           // MC mother where all final state candidates have a reconstructed track (--> have a mc particleID != 0) (loop over gen)
                kHFStepAcceptanceTrackable, // MC mother where all final state candidates have a reconstructed track (--> have a mc particleID != 0) and pass eta and pt selection (loop over gen)
                kHFStepItsTpcTrackableCuts, // MC mother where pion <-- charm passes isGlobalTrkWoDca selection (here we neglect all remaining selections: cascade/V0 cuts, PID, pion DCA cut) (loop over gen)
                kHFStepItsTrackableCuts,    // MC mother where pion <-- charm passes isITSQualityTrack selection (here we neglect all remaining selections: cascade/V0 cuts, PID, pion DCA cut) (loop over gen)
                kHFStepTracked,             // signal candidates which have been found by candidateCreator for the desired decay channel (loop over rec)
                kHFStepTrackedCuts,         // signal candidates which have been found and pass the candidateSelector cuts but PID (loop over rec)
                kHFStepTrackedSelected,     // signal candidates which pass the selector and pass the candidateSelector cuts including PID (PID for final state particles) (loop over rec)
                kHFNSteps };

  enum TrackableStep { kTrackableAll = 0, // all tracks
                       kTrackableITS,     // track contains ITS information (loop over gen)
                       kTrackableTPC,     // track contains TPC information (loop over gen)
                       kTrackableITSTPC,  // track contains ITS and TPC information (loop over gen)
                       kNTrackableSteps };
